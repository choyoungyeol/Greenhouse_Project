...
def update_graph():
    ...
    try:
        ...
        # Find the index corresponding to the user-input date
        user_input_date = entry_date.get()
        if user_input_date not in df['정식후날짜'].astype(str).values:
            raise ValueError("Invalid date input")
        else:
            idx = df[df['정식후날짜'].astype(str) == user_input_date].index[0]
            if idx < len(x) - 1:
                x = x[:idx+1]
                temperature = temperature[:idx+1]
            else:
                messagebox.showinfo("Warning", "Entered date is beyond available data. Please enter a date on or before {}.".format(x[-1]))

    except (pd.errors.EmptyDataError, FileNotFoundError, KeyError, IndexError, ValueError):
    ...
    y1, y2 = simulate_graph(np.arange(len(x)), cm, rm, tb)
...
# Label for the input date
label_date = tk.Label(window, text="수확일 입력", bg="#F0F0F0", font=("Arial", 12))
label_date.pack()

# Entry widget for user input
entry_date = tk.Entry(window, font=("Arial", 12))
entry_date.pack()
